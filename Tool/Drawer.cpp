...
CDrawer::CDrawer(LPDIRECT3DDEVICE9 pGraphic_Device)
	:CGameObject(pGraphic_Device)
{
}

CDrawer::CDrawer(const CDrawer & rhs)
	:CGameObject(rhs)
{
}

HRESULT CDrawer::NativeConstruct_Prototype()
{
...
HRESULT CDrawer::NativeConstruct(void * pArg)
{
...
void CDrawer::Tick(_float fTimeDelta)
{
	__super::Tick(fTimeDelta);
}

void CDrawer::LateTick(_float fTimeDelta)
{
...
HRESULT CDrawer::SetUp_Components()
{
	/* For.Com_Transform */
	...
	if (FAILED(__super::SetUp_Components(TEXT("Com_VIBuffer"), STAGE_SCENE, TEXT("Prototype_Component_VIBuffer_Drawer"), (CComponent**)&m_pVIBufferCom)))
		return E_FAIL;
...
_bool CDrawer::Picking(_float3 * _pickpos)
{
	_float3	vOut;
...
CDrawer * CDrawer::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CDrawer*	pInstance = new CDrawer(pGraphic_Device);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created CDrawer"));
...
CGameObject * CDrawer::Clone(void * pArg)
{
	CDrawer*	pInstance = new CDrawer(*this);
