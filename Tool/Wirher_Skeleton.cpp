...
void CWither_Skeleton::Tick(_float fTimeDelta)
{
...
HRESULT CWither_Skeleton::Render()
{
	...
	if (FAILED(m_pTextureCom->Bind_OnGraphicDevice(6)))
		return E_FAIL;
	...
	for (int i = 0; i < 9; ++i)
	{
		...
		if (FAILED(m_vTT[i]->Bind_OnGraphicDevice(6)))
			return E_FAIL;
...
_bool CWither_Skeleton::Picking(_float3 * _pickpos)
{
	_float3	vOut;
...
CWither_Skeleton * CWither_Skeleton::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	...
	{
		MSG_BOX(TEXT("Failed to First Created CWither_Skeleton"));
...
CGameObject * CWither_Skeleton::Clone(void * pArg)
{
	...
	{
		MSG_BOX(TEXT("Failed to Clone Created CWither_Skeleton"));
		Safe_Release(pInstance);
	}
