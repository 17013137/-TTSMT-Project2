...
void CFanatic::Tick(_float fTimeDelta)
{
...
HRESULT CFanatic::Render()
{
	...
	if (FAILED(m_pTextureCom->Bind_OnGraphicDevice(18)))
		return E_FAIL;
	...
	for (int i = 0; i < 9; ++i)
	{
		...
		if (FAILED(m_vTT[i]->Bind_OnGraphicDevice(16)))
			return E_FAIL;
...
_bool CFanatic::Picking(_float3 * _pickpos)
{
	_float3	vOut;
...
void CFanatic::Set_Part2(int Part, _float3 Pos)
{
	_float4x4	MX;
	D3DXMatrixIdentity(&MX);
...
CFanatic * CFanatic::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	...
	{
		MSG_BOX(TEXT("Failed to First Created CFanatic"));
...
CGameObject * CFanatic::Clone(void * pArg)
{
	...
	{
		MSG_BOX(TEXT("Failed to Clone Created CFanatic"));
		Safe_Release(pInstance);
	}
